...
def get_layer_data(layer, data1, data2):
    ...
    if (dataset1[layer].shape[0] == 1) or (len(dataset1[layer].shape) == 1):
        ...
        max_val = []
        if keras_output_length > layer:
            min_val.append([np.min(sub) for sub in keras_outputs[layer]])
            max_val.append([np.max(sub) for sub in keras_outputs[layer]])
        if fpga_output_length > layer:
            min_val.append([np.min(sub) for sub in fpga_outputs[layer]])
            max_val.append([np.max(sub) for sub in fpga_outputs[layer]])
        if debug_output_length > layer:
            min_val.append([np.min(sub) for sub in debug_outputs[layer]])
            max_val.append([np.max(sub) for sub in debug_outputs[layer]])
...
def make_plot(layer, view, data1, data2):
    ...
    length, dataset1, dataset2, dataset3, min_val, max_val, dataset4 = \
        get_layer_data(layer, data1, data2)
    plotgrid = []
    ...
    if length == 0:
        ...
        for channel in range(channels):
            ...
            if view == 3:
                ...
                if view == 2:
                    view_label = '{0}, {1} Difference'.format(data1select.value,
                                                              data2select.value)
                if view == 4:
                    view_label = '{0}, {1} Difference - Limited ColorRange'.\
                        format(data1select.value, data2select.value)
        ...
        if imgs[0].shape != imgs[-1].shape:
            pad_width = [(0, 0), (0, imgs[0].shape[1]-imgs[-1].shape[1])]
            imgs[-1] = np.pad(imgs[-1], pad_width=pad_width, mode='constant',
                              constant_values=0)
        imgs = np.vstack(imgs)
        imgs = imgs[::-1]
        ...
        p = figure(title=view_label + ' View. Layer: ' +
                   os.path.basename(keras_files[layer]),
                   x_range=(0, imgs.shape[1]), y_range=(0, imgs.shape[0]),
        ...
        if view == 3:
            ...
            color_mapper = LinearColorMapper(palette="Viridis256",
                                             low=-data1std/2, high=data1std/2)
        else:
            ...
            color_mapper = LinearColorMapper(palette="Viridis256",
                                             low=color_min, high=color_max)
        p.image(image=[imgs], x=0, y=0, dw=imgs.shape[1], dh=imgs.shape[0],
        ...
        color_bar = ColorBar(color_mapper=color_mapper, ticker=BasicTicker(),
                             ...
                             location=(0, 0), width=10, height=250)
        ...
        p = figure(tools=TOOLS,
                   title="Layer " + str(layer) + 'Graph View. Layer: '
                         + os.path.basename(keras_files[layer]),
                   toolbar_location="left", plot_width=1200, plot_height=800)
        source_dict = {}
        ...
        if keras_output_length > layer:
            k_line = p.line('x', 'keras', source=source,
                            legend=dict(value="Keras"),
                            line_color="red", line_width=1)
        if fpga_output_length > layer:
            f_line = p.line('x', 'fpga', source=source,
                            legend=dict(value="FPGA"),
                            line_color="blue", line_width=1)
        if debug_output_length > layer:
            d_line = p.line('x', 'debug', source=source,
                            legend=dict(value="Debug"),
                            line_color="green", line_width=1)
        if keras16_output_length > layer:
            k16_line = p.line('x', 'keras16', source=source,
                              legend=dict(value="Keras16"),
                              line_color="red", line_width=1)
...
parser.add_argument("--save_layers", type=str,
                    help="Saves all layers (can't be used with server)")
...
keras_folder = os.path.join(network_debug_folder, 'keras_outputs')
keras16_folder = os.path.join(network_debug_folder, 'keras_outputs_float16')
fpga_folder = os.path.join(network_debug_folder, 'PLACE_FPGA_DUMPS_HERE')
debug_output_folder = os.path.join(
    network_debug_folder+'fpga_dump_debug_outputs')
visual_output_folder = os.path.join(
    network_debug_folder+'visualisation_outputs')
...
layerselect = Select(
    title="Layer:", value=layer_name[layer], options=layer_name)
viewselect = Select(title="View:", value='Data1',
                    options=list(view_dict.keys()))
data1select = Select(title="Dataset 1:", value=list(
    data_dict.keys())[0], options=list(data_dict.keys()))
if len(data_dict.keys()) > 1:
    data2select = Select(title="Dataset 2:", value=list(
        data_dict.keys())[1], options=list(data_dict.keys()))
else:
    data2select = Select(title="Dataset 2:", value=list(
        data_dict.keys())[0], options=list(data_dict.keys()))
