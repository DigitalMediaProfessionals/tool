...
for layer in range(fpga_output_length):
    ...
    # output_string+="{:<10}|{:<6}|{:<8}\n".format("","", "===FPGA RMS ERROR===")
    # output_string+="{:<10}|{:<6}|{:<8}|{:<8}|{:<8}\n".format("","", "Keras", "Debug", "Keras16")
    # output_string+="{:<10}|{:<6}|{:<8}|{:<8}|{:<8}\n".format("","", fpga_keras_rmse, fpga_debug_rmse, fpga_keras16_rmse)
    # output_string+="{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","","Max","Min","Mean","StDev")
    # output_string+="{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","Keras",keras_max, keras_min, keras_mean, keras_std)
    # output_string+="{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","FPGA",fpga_max, fpga_min, fpga_mean, fpga_std)
    # output_string+="{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","Debug",debug_max, debug_min, debug_mean, debug_std)
    # output_string+="{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","Keras",keras16_max, keras16_min, keras16_mean, keras16_std)

    file.write("{:<10}|{:<6}\n".format("Layer " + str(layer),""))
    # file.write("{:<10}|{:<6}|{:<8}\n".format("","", "===FPGA RMS ERROR==="))
    file.write("{:<10}|{:<6}|{:<8}|{:<8}|{:<8}\n".format("","", "Keras", "Debug", "Keras16"))
    file.write("{:<10}|{:<6}|{:<8}|{:<8}|{:<8}\n".format("","", fpga_keras_rmse, fpga_debug_rmse, fpga_keras16_rmse))
    # file.write("{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","","Max","Min","Mean","StDev"))
    # file.write("{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","Keras",keras_max, keras_min, keras_mean, np.round(keras_std, 5)))
    # file.write("{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","FPGA",fpga_max, fpga_min, fpga_mean, fpga_std))
    # file.write("{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","Debug",debug_max, debug_min, debug_mean, debug_std))
    # file.write("{:<10} {:<6}|{:<8}|{:<8}|{:<8}|{:<8}\n".format("","Keras",keras16_max, keras16_min, keras16_mean, keras16_std))
file.close()
