...
def gen_source_conv(of, name, n, layer, quantization, transweight):
    ...
    of.write('{\n')
    of.write('  dmp_dv_cmdraw_conv_v1& conf_v1 = get_layer({0}).conv_conf_v1;\n'.format(n))
    of.write('  conf_v1.header.size = sizeof(conf_v1);\n')
    of.write('  conf_v1.header.device_type = DMP_DV_DEV_CONV;\n')
    of.write('  conf_v1.header.version = 1;\n')
    of.write('  dmp_dv_cmdraw_conv_v0& conf = conf_v1.conv_cmd;\n')
            ...
            if node.type in (NodeType.Convolution, NodeType.InnerProduct):
                ...
                if len(self.run) == 0:
                    ...
                    for node_in in node.input_nodes[:-1]:
                        if (node_in.type in (NodeType.Convolution,
                                             NodeType.Pooling,
                                             NodeType.InnerProduct)
                            and ((node_in.output_dim[-1] % 8 != 0) or
                                 (node_in.output_size !=
